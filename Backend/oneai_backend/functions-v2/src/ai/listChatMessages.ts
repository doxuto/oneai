...
import { listChatMessagesHandler } from "./handler.js";

export const listChatMessages = onCall(
  { enforceAppCheck: true, memory: "512MiB", timeoutSeconds: 120, maxInstances: 10, secrets: [OPENAI_API_KEY, GEMINI_API_KEY] },
  (request: CallableRequest<unknown>) => listChatMessagesHandler(callerOf(request), request.data, liveDeps()),
);
