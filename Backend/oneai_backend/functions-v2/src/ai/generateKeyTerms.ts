...
import { generateKeyTermsHandler } from "./handler.js";

export const generateKeyTerms = onCall(
  { enforceAppCheck: true, memory: "512MiB", timeoutSeconds: 120, maxInstances: 10, secrets: [OPENAI_API_KEY, GEMINI_API_KEY] },
  (request: CallableRequest<unknown>) => generateKeyTermsHandler(callerOf(request), request.data, liveDeps()),
);
