...
import { nearestMinutes, type Neighbour } from "./embedding.js";
...
/** What the model sees for one note: title, date, summary text and sections, clipped. */
export function noteForPrompt(minuteId: string, m: MinuteDoc, maxChars = ASK_ALL_NOTE_CHARS): string {
  const lines: string[] = [];
  const s = m.summary;
...
/** Sources actually cited as [[note:id]], in citation order; uncited candidates are dropped. */
export function citedSources(answer: string, candidates: AskAllSource[]): AskAllSource[] {
  const seen = new Set<string>();
  const out: AskAllSource[] = [];
  for (const m of answer.matchAll(/\[\[note:([A-Za-z0-9_-]+)\]\]/g)) {
    const id = m[1];
...
export async function askAllHandler(caller: Caller | undefined, raw: unknown, deps: Deps, onDelta?: (delta: string) => void): Promise<AskAllOutput> {
  ...
  try {
    ...
    if (!vector) throw new HttpsError("unavailable", "Embedding unavailable");
    const hits = await nearestMinutes(deps.db, uid, vector, ASK_ALL_NOTES, ASK_ALL_MAX_DISTANCE);
    const notes = await loadNotes(deps, uid, hits);
    const candidates = notes.map(toSource);
    const context = notes.length === 0 ? "(no matching notes)" : notes.map((n) => noteForPrompt(n.minuteId, n.doc)).join("\n\n");
    ...
    logDone("search.askAll", startedAt, { uid, candidates: candidates.length, cited: sources.length, model, tokensIn: tokens.input, tokensOut: tokens.output });
