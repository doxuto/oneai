...
| [x] | S11-13a | Tool benchmark STT: `lib/stt/wer.ts` (normalize giữ dấu, Levenshtein, WER + CER), `tools/stt-benchmark.mjs` (2 vendor thật, `<tên>.txt` tham chiếu, summary weighted WER + RTF, JSON), script `stt-benchmark` | 3 unit | **[Toan]** T19: chuẩn bị bộ file + chạy với key thật |
| [x] | S11-03b | BE ôn tập: `study/{types,schedule,handler}` (`summarize`, `localHourOn` chịu DST, `computeRemindAt`, `groupByUser`), callable `syncReviewSchedule`, job `remindReviews` (`jobs/remindReviews.ts`, collection-group `study.remindAt`, một push/user, clear sau gửi), `reviewDueCopy` en/vi/es, `NotificationPrefs.reviewReminders` + `updateNotificationPrefs` partial, index field override | 6 unit + 3 integration | app S11-03c sau |
| [x] | S11-09c | Cảnh báo pin: `battery_check.dart` (`isBatteryTooLow` thuần, ≤15 % và không sạc), snack trước lần ghi đầu, `battery_plus` | 1 test | |
| [x] | S11-01b | Trích dẫn tới mốc audio: `search/chunks.ts` (`chunkTranscript` gom lượt nói ≤350 từ, cap 200 chunk, fallback cửa sổ cố định), `embedChunks` trong `embedMinute` (xoá cũ, batch ≤400, `chunkCount`), `nearestChunks` collection-group, `askAllHandler` gộp chunk hit theo note + bù note-level, `noteForPrompt(chunks)` `[t=]`, `citedSources` `@SECONDS`, prompt; index `chunks` (uid + embedding); app `AskAllSource.startSeconds`, chip `· m:ss`, `SummaryArgs.seekSeconds` → mở player + seek | BE 5 unit + 1 integration; app test cập nhật | Toan: cùng T17 (deploy indexes) |
| [x] | S11-12b | Consent onboarding: `ConsentNotice` (SharedPreferences `RECORDING_CONSENT_ACK_V2`), `RecordAudioScreen._consentOk()` trước lần ghi đầu — StyledDialog "Trước khi ghi âm", nút phụ mở share sheet thông báo | 1 test | |
| [x] | S11-06b | Share nhiều file: `pickIncomingShares`, `incomingShareProvider` → `List`, `PendingIncomingShares.addAll/takeFirst`, UploadFileScreen "còn N file", Home `_PendingSharesBanner` | 2 test | Toan: thêm `SEND_MULTIPLE` intent-filter |
| [x] | S11-09b | **Chunk + foreground service**: app `recordingChunk` 5 phút, `_rotateChunk` (stop→start file mới, `_rotating` chặn báo ngắt giả), `RecordingState.closedParts/parts`, OS stop → chunk mới, `RecordingService` (flutter_foreground_task, microphone type, nuốt lỗi khi thiếu manifest), `UnfinishedRecording.paths` + `usableFiles()` (bỏ chunk cụt), `NewMinuteRequest.parts/isChunked`, `uploadParts` (tuần tự, progress tổng), `TranscriptionRepository.uploadPart/partPathFor`, `start(partCount)`, queue lưu `parts`, Home khôi phục nhiều chunk; BE `sourcePartPath`, `partCount` check từng part + tổng ≤300 MB, `loadSource` ghép (`lib/media/concat.ts`, ffmpeg-static runtime require, fallback PATH), `Services.concatAudio`, storage.rules `source/parts/`; PLATFORM-SETUP manifest | BE 2 unit + 1 integration; app 3+1 test | **[Toan]** T18 |
| [x] | S11-01 | **Hỏi đáp xuyên note**: `lib/llm/embeddings.ts` (Gemini batchEmbedContents / OpenAI embeddings, `dimension` cố định, clip 8k ký tự), `search/embedding.ts` (`embeddingText`, `embeddingHash` gồm dimension, `needsEmbedding`, `embedMinute` best-effort, `nearestMinutes` findNearest COSINE), trigger `onMinuteWritten` embed khi ready/đổi summary (hash chặn vòng lặp), `jobs/backfillEmbeddings` 03:30 ≤500/lần, `search/handler.ts` `askAllHandler` (prompt `ASK_ALL_*`, `noteForPrompt` clip 2.500 ký tự, `citedSources`), callable `askAll` streaming; `firestore.indexes.json` vector index (status + embedding 768); params `*_EMBEDDING_MODEL`, `EMBEDDING_DIM`. App: `AiRepository.askAll`, `AskAllController` (lịch sử ≤8 turn client-side, `stripCitations`), `AskAllScreen` (route `/askAll`, chip nguồn → Summary), nút ✨ ở Home | BE 9 unit + 5 integration; app 4 test | Toan: `firebase deploy --only firestore:indexes` (T17) |
| [x] | S11-02 | **Tìm theo nghĩa**: callable `searchNotes` (không AI call, COSINE ≤0.75, score = 1−d/2); app `semanticHitsProvider` (debounce 400 ms, ≥3 ký tự, nuốt lỗi) ghép sau kết quả text trong `filterMinutes(semanticIds:)` | 1 test app | |
| [x] | S11-06 | **Nhận file từ app khác**: `receive_sharing_intent`; `incoming_share.dart` (`pickIncomingShare` lọc audio/video/PDF theo ext + mime, `incomingShareProvider` initial+stream, `pendingIncomingShareProvider` giữ qua đăng nhập); `UploadFileScreen(sharedFile)` hiện thẻ file + nút Bắt đầu (credit gate như thường); app.dart đẩy sang `/uploadFile`; PLATFORM-SETUP: Share Extension iOS (App Group, `RSIShareViewController`, activation rule file+movie, scheme `ShareMedia-…`) + intent-filter `SEND` Android | 3 test | **[Toan]** T16: tạo target ShareExtension + App Group sau `flutter create`; Android intent-filter |
| [x] | S11-05d | **Deep link / universal link / app link**: Hosting (`firebase.json` hosting: rewrite `/s`→sharePage, `/legal`→legal, `/n/**`→open.html; `.well-known/apple-app-site-association` + `assetlinks.json` placeholder TEAMID/SHA-256; `open.html` store badge + fallback `oneai://`); BE `sharePage?format=json` + `loadShared`, callable `importSharedNote` (copy summary/transcript, không quota, idempotent, chủ note → id của họ); app routes `/s?t=` → `SharedNoteScreen` (xem, Mở PDF, Lưu vào ghi chú), `/n/:id` → Summary, redirect đăng nhập giữ `?from=` (chỉ path nội bộ), `SharesRepository` (GET JSON qua HttpClient, callable import), `AppConfig.shareBaseUrl`; PLATFORM-SETUP: entitlements + intent-filter autoVerify + FlutterDeepLinkingEnabled | BE 1 unit + 1 integration; app 2+4 test | **[Toan]** T15: điền TEAMID + SHA-256, `firebase deploy --only hosting`, entitlements/manifest sau `flutter create`, set `SHARE_BASE_URL` |
| [x] | S11-05c | Share PDF: `share/pdf.ts` (pdfkit + Noto Sans Regular/Bold nhúng từ `assets/fonts`, tiêu đề/ngày/tóm tắt/section/transcript/footer số trang), `sharePage?format=pdf` → attachment `Hop-sprint.pdf`; trang HTML có nút Download PDF | `share/{pdf,page,sharePage}.ts`, `assets/fonts/*`, 1 unit + 1 integration | pdfkit 0.20 |
| [x] | S10-07 | Legal EN + VI: `assets/legal/{privacy,terms,delete-account}.{en,vi}.html` (dữ liệu thu thập theo SDK, AI không huấn luyện, retention 7/90, free 10 phút/ngày, ads + UMP/ATT, gói qua store, xoá tài khoản, luật VN) + function `legal` (`?doc&lang`, CSS chung); app `AppConfig.legalUrl(doc, lang)` theo locale (fallback website) | `legal/legal.ts`, `assets/legal/*`, `app_config.dart`, `settings_screen.dart`, 2 unit + 2 dart test | **[Toan]** rà nội dung; T13 dùng URL này cho Play delete-account |
| [x] | S11-10a | BE glossary: `users/{uid}/glossary/{sha1(term)}`, 3 callable, rules owner-read; `mergeKeyterms` (keywords trước, glossary sau, ≤60) → `job.options.keyterms` → Gemini STT prompt + summarize prompt `{{keyterms}}` (**`keywords` trước đây không được dùng ở đâu**) | `glossary/*`, `transcribe/{handler,pipeline,types}.ts`, `lib/stt/{types,gemini}.ts`, `ai/summarize.ts`, 5 unit + 3 integration + 1 rules | |
| [~] | A7-05 | App: `GlossaryRepository` (live + callables), `GlossaryScreen` (thêm term/hint, vuốt xoá), route `/settings/glossary`, snack "Ghi nhớ X?" sau đổi tên speaker | `settings/glossary_screen.dart`, `data/{models,repositories}/glossary_*`, 2 test | chưa compile |
| [~] | A7-01 | Hạn mức minh bạch: pill "N phút" cạnh nút Upgrade ở Home (đỏ khi ≤1 phút, chạm → paywall), dòng "còn N phút · tối đa M phút/bản" trong sheet tạo note; màn ghi đã có (Q-01) | `home_screen.dart`, `new_minutes_bottom_sheet.dart` | chưa compile; paywall copy = RevenueCat (T14) |
| [~] | A7-02 | Ghi âm chống ngắt (bước 1): `record.onStateChanged` → phát hiện OS ngắt (cuộc gọi/Siri), đánh dấu `interrupted`, tự resume khi app active + snack; marker `UNFINISHED_RECORDING_V2` ghi 15 s/lần → Home banner Khôi phục/Bỏ; iOS categoryOptions; ghi chú foreground service Android + chunk còn lại trong PLATFORM-SETUP | `recorder_controller.dart`, `record_audio_screen.dart`, `home_screen.dart`, 3 test | chưa compile; foreground service Android cần native sau compile |
| [~] | A7-03 | Consent: nút "Thông báo ghi âm" trên app bar màn ghi → share sheet văn bản (có "không dùng để huấn luyện") | `record_audio_screen.dart`, l10n ×3 | |
| [~] | A7-04 | Song ngữ: `TranslateToggle` (Bản gốc · Ngôn ngữ ▾) trên tab Summary và Transcript, `TranslatedBody` stream qua `translationProvider` (cache server) | `detail/translate_toggle.dart`, `summary_tab.dart`, `transcript_tab.dart` | chưa compile |
| [x] | Q-01 | **Quota đổi sang phút/ngày, bỏ rewarded** (Toan 24/09): BE `quota.ts` theo giây (`consume` đặt cọc, `settleQuota` theo độ dài đo được trước STT, `refund` theo `chargedSeconds`), params `FREE_DAILY_SECONDS/PREMIUM_DAILY_SECONDS/PDF_CHARGE_SECONDS`, `getMe.quota {usedSeconds, limitSeconds, maxDurationSeconds}`, xoá `ads/*` + SSV tests + policy; app: `Quota` giây, credit gate không ad (dialog Premium Required), màn ghi hiện "còn N phút" + tự dừng ở trần, Settings "Free minutes today", bỏ thẻ reward | BE 3 unit + 7 integration (sửa/mới); app 7 test | rules `adRewards` giữ đóng |
| [x] | R-02 | Nghiên cứu đối thủ & thị trường (họp: 10 app; học: 15 app + 4 nền tảng; giá/chuyển đổi RevenueCat 2026; eCPM VN; than phiền 2026) → 12 việc mới: S8-13..15, S11-09..13, S12-06..09, app A7-01..06; đề xuất chốt OQ-02/03/04 | `docs/21-COMPETITOR-RESEARCH.md`, 09/18/17/11 | Toan xác nhận OQ-02/03/04 |
| [~] | S11-03 | App: SM-2 thuần (`Sm2.review/dueOrder`, lapse → 1 ngày, easy ×1.3, hard ×1.2), lịch per-device `FLASHCARD_REVIEW_V2_{minuteId}` theo câu hỏi, `ReviewSchedules` notifier; FlashcardsSheet: hàng "N thẻ đến hạn · Ôn tập" + chế độ Review (lật → 4 nút chấm, Again quay lại trong phiên, màn kết thúc) | `features/study/{sm2,review_store}.dart`, `flashcards_sheet.dart`, 5 test | chưa compile; nhắc push + sync server: sau |
| [~] | S11-07 | App: `UploadQueue` (Notifier sống cả app): enqueue khi mở màn xử lý → giữ `NewMinuteFlow` sống sau khi rời màn, retry ở cạnh online hoặc backoff 5/15/60/300s, rời hàng đợi khi server nhận job (processing) — không bao giờ retry sau `start` để không trừ 2 lần; lưu `UPLOAD_QUEUE_V2`, khôi phục sau restart nếu file còn; banner Home + "Waiting for network" ở thẻ lỗi; dialog thoát có "Back" = chạy nền | `transcription/upload_queue.dart`, `audio_processing_screen.dart`, `home_screen.dart`, `app.dart`, 5 test | chưa compile |
| [x] | S11-05a | BE: `createShareLink` (1 link/note, idempotent, đổi option → xoay token) / `revokeShareLink`; `sharePage` HTTP public render HTML chỉ đọc (escape, noindex, no-store, view counter), `shares/{token}` server-only (rules + test), xoá note/xoá user dọn link; `getMinute.share`; param `SHARE_BASE_URL` | `share/*`, `minutes/{handler,_shared,types}.ts`, `users/lifecycle.ts`, 6 unit + 4 integration + 1 rules | không cần Hosting |
| [~] | S11-05b | App: menu Share → "Share link" (dialog chọn kèm transcript) / "Copy link" / "Revoke link", `ShareInfo` trong `MinuteDetail` | `summary_screen.dart`, `minutes_repository.dart`, `minute_models.dart` | chưa compile |
| [x] | S11-04a | BE: `translate` streaming (summary → text có heading; transcript theo speaker, chia khối ≤6k ký tự), cache theo `(part, lang, sourceHash)`, 1 AI call | `ai/{translate,translation}.ts`, `prompts/ai.ts` TRANSLATE_SYSTEM, 4 unit + 2 integration | |
| [~] | S11-04b | App: `AiRepository.translate` stream, `TranslationController`, `TranslationSheet` (Summary/Transcript + ngôn ngữ + copy) từ hàng Study tools | `ai_tools/translation_sheet.dart`, 2 test | chưa compile |
| [x] | S11-08a | BE: `startTranscription.template` (6 giá trị, mặc định auto) → job/minute; `prompts/templates.ts` `TEMPLATE_GUIDANCE` chèn khối TEMPLATE vào prompt summarize; `MinuteDetail.template` | `prompts/templates.ts`, `ai/summarize.ts`, `transcribe/*`, 3 unit test | không đổi schema output |
| [~] | S11-08b | App: chips template trong PromptLanguageSheet (TagChip), `TranscriptionOptions.template`, l10n 6 key ×3 | `prompt_language_sheet.dart`, `transcribe_models.dart`, 2 test | chưa compile |
...
| T19 | S11-13: tạo `functions-v2/bench/` với ≥10 file VI (bắc/trung/nam, có thuật ngữ EN) + `.txt` transcript chuẩn; chạy `ELEVENLABS_API_KEY=… GEMINI_API_KEY=… npm run stt-benchmark -- ./bench`; chọn `STT_VENDOR` theo WER + RTF, ghi kết quả vào `13-CONFIG-INVENTORY.md` | S11-13 |
| T18 | S11-09: `cd functions-v2 && npm install` (tải binary `ffmpeg-static`, cập nhật package-lock — agent bị proxy chặn 407) rồi commit lock; sau `flutter create` thêm permissions + `<service>` `flutter_foreground_task` theo PLATFORM-SETUP; `firebase deploy --only storage` (rule `source/parts/`) | S11-09b |
| T17 | `firebase deploy --only firestore:indexes` (vector index cho `embedding`) trước khi bật S11-01/02; lần đầu chạy `backfillEmbeddings` bằng tay (Cloud Scheduler › Run now) để embed note cũ | S11-01 |
| T16 | Share Extension iOS: target `ShareExtension` + App Group `group.top.doxutostudio.one.ai` + Podfile theo PLATFORM-SETUP §Nhận file; Android intent-filter `SEND` audio/video/pdf | S11-06 |
| T15 | Deep link: thay `TEAMID` trong `hosting/.well-known/apple-app-site-association`, 2 SHA-256 trong `assetlinks.json` (Play Console › App integrity), `firebase deploy --only hosting`; sau `flutter create` áp entitlements + intent-filter theo PLATFORM-SETUP; đặt `SHARE_BASE_URL=https://<project>.web.app/s` | S11-05d |
