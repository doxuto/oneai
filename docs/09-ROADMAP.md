...
| S11-01b | Trích dẫn tới mốc audio — ✅ BE: chunk transcript ~350 từ theo lượt nói (`chunks/{order}`, `chunkCount`), embed batch, collection-group vector index, `askAll` ưu tiên chunk hit + `[[note:ID@SECONDS]]` → `sources[].startSeconds`; ✍️ app: chip nguồn hiện `· 0:45`, mở tab Transcript và tua tới đó (`SummaryArgs.seekSeconds`) | |
| S11-03 | **Ôn tập flashcard theo lịch** (SM-2), nhắc qua push đã có — ✍️ app: `Sm2` thuần + `ReviewStore` (per device) + chế độ Review trong FlashcardsSheet (Again/Hard/Good/Easy); ✅ BE: `syncReviewSchedule` (lưu `study/review`, `remindAt` 19:00 giờ máy) + job `remindReviews` mỗi giờ → push "N thẻ đến hạn" gộp theo user, không nag; pref `reviewReminders`; ✍️ app: sync sau mỗi phiên + toggle Settings (S11-03c) | |
...
| S11-13 | **Benchmark tiếng Việt** (Ops, sau deploy dev) — ✅ tool `npm run stt-benchmark -- ./bench` (WER/CER/latency/RTF, JSON) — **[Toan]** thu 10 file VI bắc/trung/nam lẫn thuật ngữ EN + transcript chuẩn, chạy → chọn `STT_VENDOR`; công bố "độ chính xác tiếng Việt" trên listing — không đối thủ nào làm | bảng số đo trong docs |
